...
//                    TopScoreDocCollector.createSharedManager(request.getResultSize(), null, 600000));
    ...
    private static Query buildBoolQueryWithDivide(MatchSearchRequest request, int index, int numThread) {
...
//        booleanQueryBuilder.add(LatLonPoint.newDistanceQuery(RoomIndexKeyWord.RECOMMEND_LOC, request.getLat(), request.getLon(), 500000), BooleanClause.Occur.FILTER);
        ...
        booleanQueryBuilder.add(DoublePoint.newRangeQuery(RoomIndexKeyWord.RECOMMEND_FACE_SCORE, offset, offset + step), BooleanClause.Occur.FILTER);
    ...
    private static void result(MatchSearchResponse response, IndexSearcher search, ScoreDoc[] scoreDocs) throws IOException {
